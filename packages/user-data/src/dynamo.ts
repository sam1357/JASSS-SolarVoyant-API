...
/**
 * This function clears the notifications for a given user
 * @param table the table to write to in DynamoDB
 * @param user_id the user_id provided by the request
 * @returns nothing
 */
export const clearNotifications = async (table: string, user_id: string) => {
  validateTableName(table);
  await setUserInfo(table, user_id, {
    notifications: [],
  });
};

export const GetUserAll = async (table: string, userID: string) => {
  validateTableName(table);
  ...
  try {
    ...
    if (response.Item === undefined) {
      throw new ErrorWithStatus("UserID does not exist", 400);
    }
    // Not including the password_hash in the response
    if (response.Item.password_hash) {
      delete response.Item.password_hash;
    }

    if (response.Item.token_hash) {
      delete response.Item.token_hash;
    }
  } catch (err) {
    throw new ErrorWithStatus("Unable to read user", 400);
  }
...
export const setUserInfo = async (
  ...
  Object.entries(info).forEach(([field, value]) => {
    validateUserFields(field, value);
  });
...
const validateUserFields = (field: string, value: any) => {
  // Various regex statements
  const SARegex = /^\d+(\.\d{1,5})?$/;
  ...
  if (field.toLowerCase() === "surface_area") {
    // Validating surface area
    if (!SARegex.test(value)) {
      throw new ErrorWithStatus(`${field} must be a positive number (up to 5 decimal places)`, 400);
    }
  } else if (
    field.toLowerCase() === "quarterly_energy_consumption" ||
    field.toLowerCase() === "quarterly_energy_production"
  ) {
    // Validating energy consumption/production
    value = value.replace(/\s/g, "");
    const numArray = value.split(",");
    if (numArray.length !== 4) {
      throw new ErrorWithStatus(`${field} must consist of four comma separated numbers`, 400);
    }
    for (const num of numArray) {
      if (!SARegex.test(num)) {
        throw new ErrorWithStatus(`${num} is not a positive number (up to 5 decimal places)`, 400);
      }
    }
    return;
  }

  if (field.toLowerCase() === "receive_emails") {
    if (value !== "true" && value !== "false") {
      throw new ErrorWithStatus("receive_emails field must be set to 'true' or 'false'", 400);
    }
  }

  if (field.toLowerCase() === "lower_limit" || field.toLowerCase() === "upper_limit") {
  ...
  if (
    field.toLowerCase() === "temp_coefficient" ||
    field.toLowerCase() === "daylight_coefficient"
  ) {
  ...
  if (field.toLowerCase() === "production_coefficient") {
    if (!Array.isArray(value)) {
      throw new ErrorWithStatus("production_coefficient must be an array", 400);
    }

    if (value.length !== 4) {
      throw new ErrorWithStatus("production_coefficient must have four entries", 400);
    }

    for (const entry of value) {
      if (!floatRegex.test(entry)) {
        throw new ErrorWithStatus("production_coefficient must have number entries", 400);
      }
    }
  }

  if (field.toLowerCase().startsWith("q")) {
    if (!floatRegex.test(value)) {
      throw new ErrorWithStatus("Q fields must be numbers", 400);
    }
  }
};
      ...
      if (item.email === email) {
        if (item.provider) {
          throw new ErrorWithStatus(
            ...
            `You previously signed up using ${item.provider}. So you do not have a SolarVoyant password to reset.`,
            403
          );
        }
...
export function validateTableName(table: string) {
  if (table !== tableName && table !== testTableName) {
  ...
  const htmlContents: string = `
    <div align="center">
      ...
      <p style="${textStyle} font-size: medium;">
        Please copy this token into SolarVoyant to reset your password. If you didn't request a 
