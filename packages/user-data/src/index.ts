...
export async function handler(event: LambdaFunctionURLEvent): Promise<APIGatewayProxyResult> {
  ...
  if (httpMethod === "POST" && isCorrectPath(path, "clear-notifications")) {
    return handleClearNotifications(event);
  }
  ...
  if (httpMethod === "GET" && isCorrectPath(path, "get-all")) {
    return handleGetAll(event);
  }
